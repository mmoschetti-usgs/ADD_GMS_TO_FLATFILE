...
// header lines
      ...
      while( fgets(buff2,BUFFLEN,fpAddToFlatFile) ) {
        ...
        assign_cols_flatfile(columns, &stLat2, &stLon2, &evMag2, &evLon2, &evLat2, &evDep2, &evYear2, &evMon2, &evDay2, &evHour2, &evMin2, &evSec2);
        free(columns);
...
// write modified flatfile
    if ( objMisfit < 25 ) {
      fprintf(stderr,"%.3f %.3f %.3f %.3f %.2f %.2f\n", stLat, stLat2, stLon, stLon2, evMag, evMag2);
