...
/*--------------------------------------------------------------------------*/
{
  ...
  float lonF, latF, depF, epiF, hypoDistF, lonHD, latHD, depHD, epiHD, hypoDistHD, lonHypoDD, latHypoDD, depHypoDD, epiHypoDD, hypoDistHypoDD, uncertEpiHD, uncertDepthHD, northingErr, eastingErr, depthErr;
...
// assign values from min-misfit event
      ...
      strip(buff);
      fprintf(fpFlatFileMod,"%s,%.3f,%.3f,%.3f,%.2f,%.2f,",lonF,latF,depF,epiF,hypoDistF);
...
// HD locations
      if ( catalogStruct[matchEle].locationHD ) {
        ...
        fprintf(fpFlatFileMod,"%.3f,%.3f,%.1f,%.1f,%.1f,%.1f,%.1f,", lonHD,latHD,depHD,uncertEpiHD,uncertDepthHD,epiHD,hypoDistHD);
Event Longitude HD,Event Latitude HD,Event Depth HD (km),Uncert. epi HD,Uncert. Depth HD (km),R_{epi} HD (km),R_{hypo} HD (km),
      }
//      fprintf(stderr,"Flatfile: %d %d\n", epochTimeFlatFile, catalogStruct[matchEle].epochTime);
      if ( writeEventOutput ) {
