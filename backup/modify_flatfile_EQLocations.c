...
/*--------------------------------------------------------------------------*/
{
  ...
  float lonF, latF, depF, epiF, hypoF, lonHD, latHD, depHD, epiHD, hypoHD, lonHypoDD, latHypoDD, depHypoDD;
...
//      fprintf(fpErrorLog,"%.2f %d %.1f %.1f\n", magMisfit, (int)timeMisfit, distMisfit, objMisfit);
      if ( (magMisfit<0.2) && (timeMisfit<10) && (distMisfit<10) ) {
        ...
        fprintf(fpErrorLog2,"%.2f %d %.1f %.1f\n", magMisfit, (int)timeMisfit, distMisfit, objMisfit);
      }
      cnt1++;
    }
  }
...
{
//
  *stLat=atof(columns[6]);
  *stLon=atof(columns[7]);
